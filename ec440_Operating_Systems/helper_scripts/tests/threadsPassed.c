...
void pthread_exit(void *value_ptr){// used to exit a thread 
	...
	printf("%d just exited on line  -27 \n",CurrentThread);
	printSomeThreads();
	if(TCB[CurrentThread].threadID == 0){// checks to see if it is main. broken 
		...
		if(TCB[CurrentThread].wantedBy>-1){// problem if multilple treads joining on the same thead.  
			TCB[CurrentThread].waiting = 0;
			int joinToThread = TCB[CurrentThread].wantedBy;
			TCB[CurrentThread].wantedBy =-1;

			TCB[joinToThread].value_waitedOn = value_ptr;
			TCB[joinToThread].waiting = 0;

			printf("kato = %ld \n",(unsigned long int)(TCB[joinToThread].value_waitedOn));
		}else{
			printf("not joined...\n gross");
		}
	...
	printf("wait what the fuck nothings changed?");
...
void intialize(int id,void *arg,void *(*funct) (void *)){// intializes all but main
	...
	TCB[id].waiting = 0; // not waiting on another proeccess
	TCB[id].wantedBy =-1;
...
int pthread_join(pthread_t thread, void **value_ptr){
	...
	TCB[CurrentThread].waiting = 1;
	int retVal =-1; 
	if(TCB[thread].wantedBy == -1) {
		retVal = 0;
		TCB[thread].wantedBy = pthread_self();
		printf("what is the thread %x \n",(unsigned int)pthread_self());
	}
	else{ 
		retVal = 1;
		perror("this thread is already being joined\n");
	} 
	unlock();
	pause();
	if(value_ptr != NULL){
		*value_ptr=(TCB[CurrentThread].value_waitedOn);
	}// here is where you give the stuff; 
	return retVal; 
}
...
void scheduler(){ // preserve current state and then pick new process to run 
	if(!setjmp(TCB[CurrentThread].place)){// save current threads place so that it can return this function into itself
		...
		for(i = newstart;i<ThreadN;i++){// goes through the thread list to find the next qued thread 
			...
			if(TCB[i].status ==1 && TCB[i].waiting == 0){//&&!TCB[i].waiting
		...
		for(i =0 ;i<ThreadN&&!checkDone;i++){// check for qued process from beging, used to go till newstart-1 but I took that out durring a debug
			...
			if(TCB[i].status == 1  && TCB[i].waiting == 0){//&&TCB[i].waiting == 0;
